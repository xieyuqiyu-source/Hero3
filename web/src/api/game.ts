...
export const gameApi = {
  ...
  getMilitaryView(playerId: string) {
    return api.get<Pick<GameState, 'army' | 'recruitQueues' | 'resources' | 'cityGold' | 'buildings' | 'activeModifiers' | 'general' | 'generals' | 'generalAssignments' | 'foodPressure' | 'serverTime'>>(
  ...
  deletePlayer(playerId: string) {
    return api.delete<PlayerDeletionResult>(`/players/${playerId}`)
  },

  /** 取消存档删除 */
  restorePlayerDeletion(playerId: string) {
    return api.post<PlayerDeletionResult>(`/players/${playerId}/restore-delete`, {})
  },
