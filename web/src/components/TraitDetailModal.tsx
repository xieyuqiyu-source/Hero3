    ...
    <div className="fixed inset-0 z-[9500] flex items-center justify-center p-4">
      ...
      `}>
        ...
        <div className="px-4 py-4 bg-gradient-to-r from-amber-500/15 to-amber-500/5 border-b border-amber-500/20">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-amber-600/70 mt-0.5">{trait.trigger}</p>
            </div>
            <button
              type="button"
              onClick={handleClose}
        ...
        <div className="px-4 py-4 space-y-3">
          {/* Bullets */}
          {trait.details.bullets.filter(b => b.label !== '爽点').length > 0 && (
            <div className="space-y-2">
              {trait.details.bullets
                .filter(b => b.label !== '爽点')
                .map((b, i) => (
                  <div key={i} className="flex gap-2 text-xs">
                    <span className="flex-shrink-0 px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-600 font-medium text-[10px]">
                      {b.label}
                    </span>
                    <span className="text-[var(--color-text-secondary)] leading-relaxed">{b.text}</span>
                  </div>
                ))}
            </div>
          )}
