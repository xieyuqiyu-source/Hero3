...
test('世界地图前端使用半径 100 全量缓存第一版玩家城池', () => {
  ...
  assert.match(source, /gameApi\.getWorldMapView\(activePlayerId, \{ radius: WORLD_MAP_FULL_LOAD_RADIUS \}\)/)
...
test('世界地图顶部展示行军状态摘要', () => {
  ...
  assert.match(source, /gameApi\.listSentReinforcements\(activePlayerId\)/)
  assert.match(source, /gameApi\.listReceivedReinforcements\(activePlayerId\)/)
...
test('世界地图主画布保持正方形格子', () => {
  ...
  assert.match(source, /factionCityClass\(target\.faction\)/)
  assert.match(source, /worldMapGrassCellClass\(cell\)/)
...
test('世界地图主画布展示中心轴和距离辅助环', () => {
  const source = readFileSync(new URL('../src/pages/map/components/WorldMapGrid.tsx', import.meta.url), 'utf8')
  assert.match(source, /<MapCompassRose \/>/)
  assert.match(source, /const MapCompassRose: FC = \(\) =>/)
  assert.match(source, /pointer-events-none absolute left-3 top-3 z-30/)
  assert.match(source, /aria-hidden="true"/)
  assert.match(source, />北<\/span>/)
  assert.match(source, />南<\/span>/)
  assert.match(source, />西<\/span>/)
  assert.match(source, />东<\/span>/)
...
test('世界地图顶部展示当前指向格子的坐标方位和距离', () => {
  ...
  assert.match(source, /const \[hoveredCell, setHoveredCell\] = useState<GridPosition \| null>\(null\)/)
  assert.match(source, /const inspectedCell = hoveredCell \?\? selectedCell \?\? focusedTargetPosition/)
  ...
  assert.match(source, /onPointerEnter=\{\(\) => setHoveredCell\(cell\)\}/)
  assert.match(source, /onPointerEnter=\{\(\) => setHoveredCell\(target\.position\)\}/)
  assert.match(source, /onPointerEnter=\{\(\) => setHoveredCell\(view\.self\)\}/)
...
test('世界地图草地格和城池格使用同一个坐标网格', () => {
  ...
  assert.match(source, /className=\{`relative h-full w-full border border-white\/20 p-0/)
  assert.match(source, /title=\{`草地坐标格 \(\$\{cell\.x\}, \$\{cell\.y\}\)`\}/)
  assert.match(source, /style=\{\{ gridColumn: cell\.x - minX \+ 1, gridRow: cell\.y - minY \+ 1 \}\}/)
  ...
  assert.match(source, /isSameGridPosition\(selectedCell, cell\) &&/)
  assert.match(source, /absolute inset-x-0 bottom-0 z-30 truncate bg-amber-400\/90/)
  assert.match(source, /\{cell\.x\},\{cell\.y\}/)
  ...
  assert.match(source, /bg-transparent shadow-inner/)
...
test('世界地图远景城池自动隐藏拥挤角标', () => {
  const source = readFileSync(new URL('../src/pages/map/components/WorldMapGrid.tsx', import.meta.url), 'utf8')
  assert.match(source, /import \{ Castle, House, LocateFixed, Minus, Plus \} from 'lucide-react'/)
  assert.match(source, /const showCityDetail = showCellCoordinate/)
  assert.match(source, /showCityDetail \? <House size=\{12\} strokeWidth=\{2\.5\} aria-hidden="true" \/> : <span aria-hidden="true">我<\/span>/)
  ...
  assert.match(source, /self \? \(showCityDetail \? <House size=\{12\} strokeWidth=\{2\.5\} aria-hidden="true" \/> : <span aria-hidden="true">我<\/span>\) : showCityDetail && <Castle size=\{12\} strokeWidth=\{2\.5\} aria-hidden="true" \/>/)
})
...
test('世界地图入口只渲染世界地图主视图', () => {
  ...
  assert.match(source, /<WorldMapTab \/>/)
  assert.match(indexSource, /export \{ default \} from '\.\/MapPage'/)
  assert.doesNotMatch(source, /PlayerTargetsTab/)
  assert.doesNotMatch(source, /NpcCityTab/)
  assert.doesNotMatch(source, /DungeonTab/)
  assert.doesNotMatch(source, /MiniGamesTab/)
  assert.doesNotMatch(source, /useState<.*Tab/)
})
