...
import type { AccountInfo, BuildingBalanceConfig, UnitsConfig } from '../api/types'
...
const props = defineProps<{ account: AccountInfo; game: GameStateStore; buildingConfigs: Record<string, BuildingBalanceConfig>; unitsConfig: UnitsConfig; cityGoldPerSecond: number }>()
const emit = defineEmits<{ logout: []; retry: []; refreshMilitary: []; upgrade: [buildingId: string]; fillResources: []; recruit: [unitId: string, amount: number]; instantRecruit: [queueId: string] }>()
...
/** 在已实现的城池页和军事征兵复刻页之间切换。 */
function selectPrimary(index: number) {
  if (index === 0 || index === 1) activePrimaryIndex.value = index
}
...
<template>
  <div class="page-shell">
    <main class="game-canvas" :aria-label="activePrimaryIndex === 1 ? '武林三国风格军事征兵页' : '武林三国风格当前城池资源页'">
      ...
      <button type="button" class="game-refresh" :disabled="game.phase === 'loading'" @click="emit('retry')">{{ game.phase === 'loading' ? '读取中' : '刷新' }}</button>
      <div class="game-body">
        ...
        <SidePanel v-if="model" :model="model" :received-at="game.receivedAt" />
