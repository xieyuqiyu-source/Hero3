...
const emit = defineEmits<{ retry: []; navigate: [x: number, y: number]; home: []; dispatch: [action: WorldMapMarchAction, targetPlayerId: string, troops: Record<string, number>, generalIds: string[]]; loadNpc: [playerId: string]; refreshNpc: []; dispatchNpc: [action: NpcCommandAction, npcId: string, troops: Record<string, number>, generalIds: string[]]; loadDungeon: [playerId: string]; startDungeon: [level: number]; fightDungeon: [waveId: string, troops: Record<string, number>, generalIds: string[]]; resetDungeonBonus: [waveId: string]; settleDungeon: []; exitDungeon: []; loadMirage: [playerId: string]; gambleMirage: [unitType: string, amount: number, betId: string, exactNumber: number]; spinMirage: [unitType: string, amount: number]; redeemMirage: [recordId: string, amount: number]; redeemAllMirage: [gameType: MirageGameType] }>()
...
<template>
  <section class="main-stage map-stage" aria-label="世界地图页">
    ...
    <div class="panel-center map-panel-center">
      ...
      <DungeonStage v-else-if="activeMapTab === '副本'" :dungeon="dungeon" :player-id="currentPlayerId" :account-gold="accountGold" :units="units" :generals="generals" :assignments="assignments" :server-time="serverTime" :received-at="receivedAt" @load="emit('loadDungeon', currentPlayerId)" @start="emit('startDungeon', $event)" @fight="(waveId, troops, generalIds) => emit('fightDungeon', waveId, troops, generalIds)" @reset-bonus="emit('resetDungeonBonus', $event)" @settle="emit('settleDungeon')" @exit="emit('exitDungeon')" />
