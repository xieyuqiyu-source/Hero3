...
watch(
  ...
  ([phase, playerId]) => {
    if (phase === 'game' && playerId) void playerGameState.load(playerId)
    else playerGameState.clear()
  },
)
</script>
...
<template>
  ...
  <GameShell v-else-if="state.phase === 'game' && state.account && state.currentPlayer" :account="state.account" :game="playerGameState.state" :building-configs="state.bootstrap?.balance.buildings ?? {}" :units-config="state.bootstrap?.units ?? {}" :city-gold-per-second="state.bootstrap?.balance.cityGoldPerSecond ?? 120" @retry="playerGameState.refresh" @refresh-military="playerGameState.refreshMilitary" @upgrade="playerGameState.upgradeBuilding" @fill-resources="playerGameState.fillResourcesPaid" @recruit="playerGameState.recruit" @instant-recruit="playerGameState.instantCompleteRecruit" @logout="logout" />
</template>
