...
根目录可一键启动 Go 后端、两个玩家前端和 GM 后台：

```bash
./dev.sh
```

- React 玩家端：`http://localhost:5173`
- GM 后台：`http://localhost:5174`
- 武林三国 Vue 玩家端：`http://localhost:5175`
...
- 配置 `HERO3_DATABASE_DSN` 后启用 MySQL/MariaDB；开发环境默认随服务启动执行轻量迁移，生产环境的 `hero3-server` 默认跳过启动迁移，结构变更应优先走面向本次功能的小迁移命令，例如黄巾起义使用 `/opt/hero3/go/bin/hero3-dbtool migrate-yellow-turban`。
...
- 生产自动部署由 GitHub Actions SSH 调度，服务器通过本地 `mihomo` 代理 `127.0.0.1:7890` 在 `/opt/hero3/source` 拉取指定提交，然后在本机构建 Go 后端、`hero3-dbtool`、React `web`、Vue `web-wlsg` 和 GM 后台；`hero3.ccoos.cn` 固定发布 `web`，`wlsg.ccoos.cn` 固定发布 `web-wlsg`，两个站点共用 Hero3 API，Actions 不构建 release 产物。
- 战报清理默认按差异化保留执行：普通/NPC/扫荡 72 小时，PVP/玩家城池来源、防守/侦查 168 小时，玩家软删除 24 小时；有效分享链接会被保护。
- 战报系统采用标准详情契约：`sourceType / battleType / viewType / winnerSide / ownerSide / ownerOutcome` 分离来源、动作、视角和当前玩家结果；玩家端军情列表支持进攻、防守、协防、侦查、扫荡分类，详情页等待完整接口返回后渲染 `detail.primarySide / secondarySide / extra`，并由服务端按 `visibility` 脱敏敌军兵力、资源、武将、特性和城防。战报参战方按“进攻方 → 交战 → 防守方 → 增援方”纵向排列，只展示出动/阵亡；每名参战将领独立保存本场获得经验和升级前后等级，防守与增援视角复制同一客观结果，历史战报按 `eventId` 从同事件其他视角补齐，敌方将领不可见时经验也同步隐藏。增援快照固定保存战斗开始时的出动兵力和逐队武将经验，同玩家或同来源城池的多批增援会合并兵力、战损与对应武将经验，且每个聚合参战方最多展示一名武将。内部详情必须登录并校验玩家归属，匿名分享只允许使用分享 token 且直接读取公开接口；批量删除复用当前列表的 `viewType / battleType` 筛选。GM 后台“战报排查”可按事件 ID 查看同一战斗下所有玩家视角战报和参与方原始快照。
...
- 黄巾起义已作为地图玩法模块接入：千帐营提供口粮承载上限，军事视图和左侧军队卡片显示当前口粮/上限；黄巾城池以 `yellow_turban` 假玩家势力展示在世界地图，不写入普通玩家表、排行榜或联盟。黄巾来袭使用独立 `yellow_turban_marches` 队列保存兵力快照，到达后复用战斗引擎结算防守战报，不掠夺、不返程、战后消失；GM 后台可编辑 `game_configs.yellow_turban` 并手动触发全服检测。
- 万象幻境当前包含仙池垂钓、军营豪赌和天机轮转；`web-wlsg` 已开放军营豪赌与天机轮转，真实接入服务端随机结算、兵力扣除、`minigame_records` 奖励库存及单条/一键兑换。仙池垂钓的文件配置只作为模板，线上 GM 修改写入数据库 `game_configs.fishing`，避免发布覆盖运营配置；钓鱼鱼饵支持独立 `rarityWeights`、最低/最高品质、神话 `mythic` 池和咬钩等待倍率，旧 `rarityBoost` 仅作为兼容兜底。天机轮转配置仍在 `go/config/slot.json`，GM 后台可调整每线押注、图案权重、倍率、免费旋转和宝匣倍率。天机轮转第二版采用每线押注、固定 5 线、3x3 服务端结果矩阵，并支持 Wild、Scatter 免费旋转和 Bonus 奖励。
...
- 轮回绝境副本使用 `go/config/reincarnation.json` 配置层级、波次、加成、金币重置随机加成价格和奖励；`web-wlsg` 的“地图 -> 副本”已接入七档难度、十八波挑战、真实兵力与武将选择、进攻/防守结算、加成重置和最终领奖，GM 后台可编辑 JSON 配置、查看实例并处理异常结算。
...
- [项目整理与模块化理解](./docs/架构/项目整理与模块化理解-2026-07-18.md)：整理当前目录、后端分层、核心边界、玩法模块和模块化程度判断。
