...
- 地图页签回归已修复：确认世界地图提交曾把原有地图入口整体替换，后续临时恢复版本使用了新按钮样式且遗漏“据点”。现已按历史版本恢复 NPC、玩家、据点、副本、万象幻境五个带图标页签和横向拖动样式，玩家页签继续承载新的世界地图并默认选中；单元测试、ESLint、生产构建和浏览器切换验证均通过。
- 世界地图首屏性能继续优化：定位到后端每次地图读取都会遍历全服账号并逐个补坐标，同时前端用一个 `Promise.all` 等待地图、行军和两类增援。现改为后端仅按当前坐标范围索引查询玩家城池轻量投影，普通读取只兜底当前查看者坐标；前端近景响应单独决定首屏，行军/增援后台加载，全图缓存延后 1.2 秒启动。历史缺失坐标继续使用现有 `backfill-world-positions` 分批命令，不在用户请求链路执行全服迁移。验证：Go 应用、仓储、HTTP 和 dbtool 测试通过；前端 72 项测试、ESLint、生产构建通过；浏览器模拟 12 个行军/增援请求持续未返回时，近景地图仍结束加载并显示附近城池，控制台无错误。
