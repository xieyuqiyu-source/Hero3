...
NPC 手动刷新通过账号资产事务原子扣除 100 账户金币并重新生成当前存档的 12 座 NPC 城池；余额不足时金币与 NPC 状态均保持不变，成功响应返回实际消耗与最新账户金币供前端同步。

仓库容量扩容使用 `GET /api/v1/city/boost/prices` 返回配置化倍率、时长组合价格，并通过 `POST /api/v1/city/capacity-boost` 在资源资产事务中扣除城金、重算容量和到期时间；响应返回实际费用，供前端按权威结果同步。
...
- `general_combat.go`：武将特性接入战斗特性总线和战报结果；真实战斗每次最多 1 名参战武将，NPC/PVP/副本按显式携带或守城主将触发。标准战报保留 `OwnerSide / OwnerGeneralID / Detail`，供前端按参战武将展示真实触发阶段、比例和逐兵种效果。
- `service_combat.go`：NPC 战报以战斗结算结果中的 `UnitLoss.Count / Losses` 分别固化战前兵力和实际阵亡；守军总战损低于 `25%` 时隐藏详情，达到阈值后才向玩家展示这两个独立快照。
...
- `service_minigame.go`：万象幻境记录、鱼饵消耗、军营豪赌结算、天机轮转 3x3 固定 5 线结算、奖励兑换和兑换事件。
...
配置 `HERO3_DATABASE_DSN` 后，服务使用 MySQL/MariaDB。开发环境默认在启动时执行当前需要的轻量迁移；生产环境默认不在 `hero3-server` 启动时执行迁移，结构变更应优先走面向本次功能的小迁移命令，例如黄巾起义使用服务器上的 `hero3-dbtool migrate-yellow-turban`：
