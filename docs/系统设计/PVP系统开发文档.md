...
### <span style="color:#16a34a">2.1 玩家世界坐标</span>

PVP 玩家目标页不再以全量玩家平铺列表作为长期方向，而是以世界地图坐标作为主入口。

当前第一版实现：

- 每个玩家都有稳定世界坐标，前端显示为地图点位。
- 坐标由 `playerId` 生成稳定伪随机值，旧玩家和新玩家都能立即获得固定位置。
- 世界大小默认 `2000 x 2000`。
- 目标接口支持按 `centerX`、`centerY`、`radius` 和 `limit` 读取当前视野内玩家。
- 返回目标包含坐标、区域编号和距离，前端可展示自己的城池、附近玩家、坐标和距离。

后续如果需要迁城、联盟地块、出生州、资源带、国战区域，应把坐标升级为独立权威表，例如 `player_world_positions`，并在创建玩家时写入；当前接口结构已经预留 `worldId`、`x`、`y`、`regionId`，可以平滑迁移。
