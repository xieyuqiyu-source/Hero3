...
- 标记 `is_claimed = true`。
- 返回更新后的奖励结果和玩家局部状态。
- 当前第一版已支持 `resource`、`city_gold`、`gold`。资源附件按当前容量上限发放，超出容量部分暂不转化。
...
- 收件人使用 `玩家昵称#六位编号`，避免玩家手动输入过长的 `playerId`。
- 玩家互发第一版只允许纯文本，不允许携带附件。
- 必须禁止给自己发信。
- 后续如出现骚扰问题，再补发送冷却、黑名单和举报。
...
玩家互发已经支持第一版纯文本能力，后续重点是限频、黑名单和举报。
