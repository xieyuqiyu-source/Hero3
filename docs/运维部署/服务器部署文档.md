...
当前部署方式是 GitHub Actions 上传指定提交的源码包到服务器，由服务器解压源码并完成构建，不再由 GitHub Actions 构建产物后上传。

自动部署流程：

```text
push main-core
  -> GitHub Actions checkout 指定提交
  -> GitHub Actions 上传源码包到服务器
  -> 服务器解压源码到 /opt/hero3/source
  -> 服务器构建 Go 后端
  -> 服务器构建玩家前端 web
  -> 服务器构建 GM 后台 admin
  -> 服务器备份旧版本
